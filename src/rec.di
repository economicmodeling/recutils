...
enum REC_VERSION_MAJOR = 1;
enum REC_VERSION_MINOR = 0;
enum REC_VERSION_STRING = "1.0";
...
alias rec_mset_disp_fn_t = void function(void *data);
alias rec_mset_equal_fn_t = bool function(void *data1, void *data2);
alias rec_mset_dup_fn_t = void* function(void *data);
alias rec_mset_compare_fn_t = int function(void *data1, void *data2, int type2);
...
enum REC_FNAME_RE = "[a-zA-Z%][a-zA-Z0-9_]*";

enum REC_TYPE_NAME_RE = "[a-zA-Z][a-zA-Z0-9_-]*";
enum REC_URL_REGEXP = "(file|http|ftp|https)://[^ \t]+";
enum REC_FILE_REGEXP = "(/?[^/ \t\n]+)+";
...
const(char*) rec_std_field_name (rec_std_field_e std_field);
...
struct rec_fex_s {}
alias rec_fex_t = rec_fex_s*;
struct rec_fex_elem_s {}
alias rec_fex_elem_t = rec_fex_elem_s*;
...
enum REC_FNAME_FEX_RE     =  REC_FNAME_RE ~ "(\\." ~ REC_FNAME_RE ~ ")?";
enum REC_FNAME_LIST_RE    =  REC_FNAME_RE ~ "([ \n\t]+" ~ REC_FNAME_RE ~ ")*";
enum REC_FNAME_LIST_CS_RE =  REC_FNAME_FEX_RE ~ "(," ~ REC_FNAME_FEX_RE ~ ")*";
enum REC_FNAME_SUB_RE     =  REC_FNAME_FEX_RE ~ "(\\[[0-9]+(-[0-9]+)?\\])?";
enum REC_FEX_FUNCTION_NAME = "[a-zA-Z_][a-zA-Z0-9_]*";
enum REC_FEX_CALL         =  REC_FEX_FUNCTION_NAME ~ "\\(" ~ REC_FNAME_FEX_RE ~ "\\)";
enum REC_FNAME_LIST_SUB_ELEM_RE = "(" ~ REC_FNAME_SUB_RE ~ "|" ~ REC_FEX_CALL ~ ")" ~ "(:" ~ REC_FNAME_FEX_RE ~ ")?";
enum REC_FNAME_LIST_SUB_RE = REC_FNAME_LIST_SUB_ELEM_RE ~ "(," ~ REC_FNAME_LIST_SUB_ELEM_RE ~ ")*";
...
rec_fex_t rec_fex_new (const char *str, rec_fex_kind_e kind);
...
bool rec_fex_check (const char *str, rec_fex_kind_e kind);
...
char* rec_fex_str (rec_fex_t fex, rec_fex_kind_e kind);
...
rec_type_kind_e rec_type_kind (rec_type_t type);
...
struct rec_record_s {}
alias rec_record_t = rec_record_s*;
...
enum REC_RECORD_TYPE_RE = REC_FNAME_RE;

/* Opaque data type representing a record set.  */
struct rec_rset_s {}
alias rec_rset_t = rec_rset_s*;
...
alias rec_aggregate_reg_t = rec_aggregate_reg_s*;
...
enum REC_F_DESCRIPTOR = 1;
enum REC_F_ICASE      = 2;
enum REC_F_UNIQ       = 4;

enum REC_Q_NOINDEX   = size_t.max;
...
enum REC_SET_ACT_NONE    = 0;
enum REC_SET_ACT_RENAME  = 1;
enum REC_SET_ACT_SET     = 2;
enum REC_SET_ACT_ADD     = 3;
enum REC_SET_ACT_SETADD  = 4;
enum REC_SET_ACT_DELETE  = 5;
enum REC_SET_ACT_COMMENT = 6;
...
struct rec_parser_s {}
alias rec_parser_t= rec_parser_s*;
...
rec_parser_t rec_parser_new (FILE *in_, const char *source);
...
alias rec_writer_mode_t = rec_writer_mode_e;
...
rec_writer_t rec_writer_new (FILE *out_);
...
void rec_writer_set_mode (rec_writer_t writer, rec_writer_mode_e mode);
...
enum REC_ENCRYPTED_PREFIX = "encrypted-";
...
bool rec_encrypt (char *in_, size_t in_size, const char *password,
                  char **out_, size_t *out_size);
...
bool rec_decrypt (char *in_, size_t in_size, const char *password,
                  char **out_, size_t *out_size);
...
alias rec_aggregate_t = char* function(rec_rset_t    rset,
                                       rec_record_t  record,
                                       const char   *field_name);
...
rec_aggregate_reg_t rec_aggregate_reg_new();
...
bool rec_aggregate_reg_add (rec_aggregate_reg_t func_reg, const char *name, rec_aggregate_t function_);
